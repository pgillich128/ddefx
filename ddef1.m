  function [sol] = ddef1(F, phi, tau, j, left, right, hIn)
% WHAT'S NEW IN THIS VERSION 
% tried to reduce rounding error in the method of computing stage
% variables--translated from \alpha to t-t_0 variables. 
% Also changed to getMesh version 4
...
%   global variables pertaining to numerical quadrature
    ...
    K = 0; %this is a global variable containing the stage values K_i
    hQuadrature = 0.1 .* (h)^(1./4); %this is the step size which will be used for numerical integration 
...
%       the RK method as the function definition
        K = F(y_next, I_numPartial + I_numStage1);
...
%   use a different function to evaluate these points.
    ...
    switch stageNumber
        case 1
            stageEval = y_n + (K).*(transformedMesh(reuseIndex : intMeshSize) - t_n);
        case 2 %this branch will never be executed in this particular script
